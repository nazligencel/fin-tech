package com.fintech.fin_tech.repositories;
...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.user.id = :userId AND t.type = :type") //COALESCE burada, sorgunun sonucunun NULL değil her zaman olmasını garanti eder
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.user.id = :userId AND t.type = :type AND YEAR(t.transactionDate) = :year AND MONTH(t.transactionDate) = :month")
    ...
    @Query("SELECT new com.fintech.fin_tech.dto.CategorySummaryDto(t.category, COALESCE(SUM(t.amount), 0)) " +
            "FROM Transaction t " +
            "WHERE t.user.id = :userId AND t.type = com.fintech.fin_tech.util.TransactionType.EXPENSE " +
            "GROUP BY t.category")
